...
from pipeline.src.submit_job import Submission
...
async def test_campaign_creation_failure(
    ...
    monkeypatch.setattr("pipeline.src.submit_job.MAX_RETRIES", 1)
    await submission.setup_session()
...
async def test_campaign_poll_failure(
    ...
    monkeypatch.setattr("pipeline.src.submit_job.MAX_RETRIES", 0)
    await submission.setup_session()
...
async def test_retries_failure(
    ...
    monkeypatch.setattr("pipeline.src.submit_job.MAX_RETRIES", 2)
